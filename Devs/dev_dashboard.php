...
if (isset($_GET['action']) || isset($_POST['action_type'])) {
    ...
    if ($action === 'get_student_badges') {
        ...
        $stmt = $conn->prepare("
            SELECT DISTINCT COALESCE(bd.criteria_key, sb.criteria_key) AS criteria_key
            FROM student_badges sb
            LEFT JOIN badge_definitions bd ON sb.badge_id = bd.badge_id
    ...
    if ($action === 'grant_badge') {
        ...
        try {
            ...
            $stmt = $conn->prepare("
                INSERT INTO student_badges (student_id, badge_id, criteria_key, date_unlocked) 
                VALUES (:student_id, :badge_id, :criteria_key, NOW())
                ON DUPLICATE KEY UPDATE date_unlocked = NOW()
            ");
            $stmt->bindParam(":student_id", $student_id, PDO::PARAM_INT);
    ...
    if ($action === 'revoke_badge') {
        ...
        try {
            $stmt = $conn->prepare("
                ...
                LEFT JOIN badge_definitions bd ON sb.badge_id = bd.badge_id
                WHERE sb.student_id = :student_id 
                AND (sb.criteria_key = :criteria_key OR bd.criteria_key = :criteria_key)
            ");
            $stmt->bindParam(":student_id", $student_id, PDO::PARAM_INT);
            $stmt->bindParam(":criteria_key", $criteria_key);
            $stmt->execute();
    ...
    if ($action === 'reset_all_badges') {
        ...
        try {
            ...
            echo json_encode(["status" => "error", "message" => "Database reset failed"]);
        }
        exit();
    }
}
?>
<!-- 2. FRONTEND HTML & JS BELOW -->
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Dark Cyberpunk Theme for SweetAlert2 */
    .swal2-popup {
      ...
      fetch(`${currentFile}?action=get_student_badges&student_id=${studentId}`)
        ...
        .then(unlockedKeys => {
          document.querySelectorAll('.badge-reset-check').forEach(cb => {
            const key = cb.value;
            updateBadgeUI(key, unlockedKeys.includes(key));
          });
          log(`SYNC COMPLETE: Student ID ${studentId} has ${unlockedKeys.length} unlocked badge(s).`);
        })
        .catch(err => {
          log(`WARN: Backend error or unreachable.`);
          Swal.fire({
            icon: 'error',
            title: 'Sync Failed',
            text: 'Could not fetch student badge data from database.',
            background: '#0b1329',
